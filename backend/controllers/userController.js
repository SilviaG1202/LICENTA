...
const createUser = asyncHandler(async (req, res) => {
  ...
  if (userExists) res.status(400).send("Utilizatorul deja exista");
...
const logoutCurrentUser = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({ message: "V-ati delogat cu succes!" });
});
...
const deleteUserById = asyncHandler(async (req, res) => {
  ...
  if (user) {
    if (user.isAdmin) {
      res.status(400);
      throw new Error("Nu se poate sterge Adminul");
    }
